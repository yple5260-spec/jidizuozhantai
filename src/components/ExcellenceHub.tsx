...
import '../excellence-scorecard.css'
...
type PhraseCategory='全部'|'催单话术'|'问题解决话术'|'优秀服务话术'

const formatDuration=(seconds:number)=>`${Math.floor(seconds/60)}:${String(seconds%60).padStart(2,'0')}`
const dimensionShort:Record<string,string>={productivity:'产能',quality:'质检',satisfaction:'满意度',fcr:'一次解决',marketing:'营销'}
...
export default function ExcellenceHub({role,state,busy,run,notify}:Props){
 ...
 const [phraseCategory,setPhraseCategory]=useState<PhraseCategory>('全部')
 ...
 const ranked=useMemo(()=>[...(excellence?.employeeAchievements||[])].sort((a,b)=>b.compositeScore-a.compositeScore||b.qualityScore-a.qualityScore||b.effectiveTaskCount-a.effectiveTaskCount),[excellence])
 ...
 const phraseCategories:PhraseCategory[]=['全部','催单话术','问题解决话术','优秀服务话术']
 const visiblePhrases=(excellence?.phrases||[]).filter(item=>phraseCategory==='全部'||item.category===phraseCategory)
 ...
 return <section className="excellence-page">
  <header className="excellence-hero">
   ...
   <div className="excellence-rule"><Target size={22}/><div><small>优秀员工评选原则</small><strong>五维指标综合评估</strong><span>{excellence.evaluation.rule}</span></div></div>
  </header>
  ...
  {tab==='employees'&&<div className="excellence-section">
   <div className="section-title"><div><span>01 / PEOPLE</span><h2>优秀员工 · 五维综合评分前 {excellence.evaluation.topPercent}%</h2><p>{excellence.evaluation.period}共评估 {ranked.length} 人；产能、质量质检、客户满意度、一次性解决率、营销达成各占20%，至少需要 {excellence.evaluation.minimumEvidenceTasks} 张有效任务单作为证据。</p></div><em>TOP {topCount}</em></div>
   <div className="achievement-method"><b>综合评分</b><span>Σ（单项实际值 ÷ 单项目标值 × 100 × 20%）</span><em>单项得分最高按120分计，避免单一指标过度拉高排名</em></div>
   <div className="achievement-table multi"><div className="achievement-head"><span>排名 / 员工</span><span>班组</span><span>综合评分</span>{['产能','质量质检','客户满意度','一次性解决率','营销达成'].map(label=><span key={label}>{label}<small>权重20%</small></span>)}<span>有效任务</span></div>{ranked.map((item,index)=><div className={`achievement-row ${topIds.has(item.id)?'top':''}`} key={item.id}><span><b>{String(index+1).padStart(2,'0')}</b><i>{item.name.slice(0,1)}</i><div><strong>{item.name}</strong><small>{item.jobNo}{topIds.has(item.id)?' · 优秀员工':''}</small></div></span><span>{item.team}</span><span className="composite-score"><strong>{item.compositeScore.toFixed(1)}</strong><i><b style={{width:`${Math.min(item.compositeScore,120)/1.2}%`}}/></i></span>{item.dimensions.map(metric=><span className={`dimension-score ${metric.actual>=metric.target?'met':'below'}`} key={metric.code}><strong>{metric.actual}{metric.unit}</strong><small>目标 {metric.target}{metric.unit}</small><em>{dimensionShort[metric.code]} {metric.score.toFixed(1)}分</em></span>)}<span>{item.effectiveTaskCount} 张</span></div>)}</div>
  </div>}
  ...
  {tab==='phrases'&&<div className="excellence-section">
   <div className="section-title"><div><span>04 / PHRASES</span><h2>优秀话术库 · 按问题场景直接调用</h2><p>不是摘一句“好听的话”，而是沉淀客户信号、推荐表达、解决步骤与禁忌表达；保留录音和质检证据。</p></div><em>{excellence.phrases.length} 条</em></div>
   <div className="phrase-category-bar">{phraseCategories.map(category=><button key={category} className={phraseCategory===category?'active':''} onClick={()=>setPhraseCategory(category)}><span>{category}</span><b>{category==='全部'?excellence.phrases.length:excellence.phrases.filter(item=>item.category===category).length}</b></button>)}</div>
   <div className="phrase-grid rich">{visiblePhrases.map((item,index)=><article key={item.id}><span>{String(index+1).padStart(2,'0')}</span><div><header><div><em>{item.category||'优秀服务话术'}</em><strong>{item.title||item.scenario}</strong></div><small>{item.scenario}</small></header>{item.customerSignal&&<p className="phrase-signal"><b>客户信号</b>{item.customerSignal}</p>}<blockquote>“{item.text}”</blockquote>{item.objective&&<p className="phrase-objective"><b>表达目的</b>{item.objective}</p>}<div className="phrase-guidance"><section><b>处理步骤</b><ol>{(item.steps||[]).map(step=><li key={step}>{step}</li>)}</ol></section><section className="phrase-avoid"><b>避免这样说</b><ul>{(item.avoid||[]).map(text=><li key={text}>“{text}”</li>)}</ul></section></div><div className="phrase-tags">{item.tags.map(tag=><i key={tag}>{tag}</i>)}</div><footer>来源 {item.sourceRecordingId} · {item.employeeName} · 质检 {item.qualityScore} 分 <b>已调用 {item.useCount} 次</b></footer></div></article>)}</div>
  </div>}
