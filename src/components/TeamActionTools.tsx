...
const taskPrefill=(member:TeamActionMember,role:Role,analysis?:TeamAttributionResult|null):TaskRequestPrefill=>{
 ...
 const recommendations=analysis?.recommendations.join('；')||'核对签入工时、员工利用率、通话均长和示忙小休率，按小时追踪改善结果。'
 return {
  ...
  problem:`${gapText}。需依据工时、员工利用率、通话均长和示忙小休率定位具体影响因素。`,
  target:`${member.name}人工应答量提升至不低于${member.responses.target??'个人目标'}通`,
  successCriteria:`系统验证人工应答量达到${member.responses.target??'个人目标'}通，并同步核对签入工时、员工利用率和通话均长是否回到个人目标。`,
...
export default function TeamActionTools({members,role,busy,run,notify}:{members:TeamActionMember[];role:Role;busy:boolean;run:Runner;notify:(text:string)=>void}){
 ...
 return <>
  ...
  {analysisOpen&&selected&&<div className="team-attribution-shade" onMouseDown={()=>setAnalysisOpen(false)}><section className="team-attribution-dialog" role="dialog" aria-modal="true" aria-labelledby="team-attribution-title" onMouseDown={event=>event.stopPropagation()}>
   <header><div><span><Bot size={16}/>AI产能归因</span><h2 id="team-attribution-title">从产能Gap拆到可管理的过程原因</h2><p>直接使用底层工时、利用率、AHT与示忙小休数据，AI负责解释，系统公式负责校验。</p></div><button aria-label="关闭归因分析" onClick={()=>setAnalysisOpen(false)}><X size={19}/></button></header>
   ...
   {loading?<div className="team-attribution-loading"><Sparkles size={28}/><h3>正在拆解产能差距</h3><p>计算目标产能，并匹配工时、利用率、通话均长和示忙小休影响</p></div>:error?<div className="team-attribution-error"><AlertTriangle size={19}/><div><strong>归因分析未完成</strong><p>{error}</p></div></div>:analysis&&<div className="team-attribution-body">
    ...
    <section className="attribution-drivers"><header><div><span>影响因素桥接</span><h3>是哪一个过程变量拉低了产能</h3></div><small>影响通数为公式桥接值；示忙小休为利用率的过程原因</small></header><div>{analysis.drivers.map((driver,index)=><article className={driver.status} key={driver.code}><b>{String(index+1).padStart(2,'0')}</b><div><span>{driver.label}</span><strong>{numberText(driver.actual,driver.unit)}</strong><small>目标 {numberText(driver.target,driver.unit)} · Gap {numberText(driver.gap,driver.unit)}</small><p>{driver.evidence}</p></div><em>{driver.impactCalls==null?'待核算':`${driver.impactCalls>0?'+':''}${driver.impactCalls}通`}</em></article>)}</div></section>
    <section className="attribution-ai"><header><span><Sparkles size={17}/></span><div><strong>{analysis.provider==='DeepSeek'?'DeepSeek归因结论':'系统规则归因结论'}</strong><small>{analysis.model}{analysis.warning?' · AI不可用时已自动降级':''}</small></div></header><p>{analysis.aiNarrative}</p><div>{analysis.recommendations.map(item=><span key={item}><CheckCircle2 size={14}/>{item}</span>)}</div></section>
   </div>}
