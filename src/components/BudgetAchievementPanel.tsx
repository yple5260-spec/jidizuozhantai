...
type Props={state:FinancialPerformanceState;role:'manager'|'director'}
...
export default function BudgetAchievementPanel({state,role}:Props){
 ...
 const grossMargin=valueFor(byCode('0601'),'actual',state.metrics),grossMarginBudget=valueFor(byCode('0601'),'budget',state.metrics)
 const afterMargin=valueFor(byCode('0603'),'actual',state.metrics),afterMarginBudget=valueFor(byCode('0603'),'budget',state.metrics)
 const maxMonth=Math.max(...revenue.budget.slice(0,6),...revenue.actual)
 const detailCodes=['01','0101','0103','01030102','01030111','0105','010502','010503','0106','0108','0110','0117','0118','06','0601','0602','0603']
 return <section className={`budget-achievement ${role}`}>
  <header><div><span><CircleDollarSign size={17}/> 预算达成 · 附件真实数据</span><h2>{state.scope} 1—6月经营预算完成情况</h2><p>预算取自《{state.sources.budget.fileName}》的“{state.sources.budget.section}”；实际取自《{state.sources.actual.fileName}》的“{state.sources.actual.section}”。</p></div><em>数据截至 {state.asOf}</em></header>
  <div className="budget-headline">
   ...
   <article className="risk"><div className="budget-kpi-label"><span><AlertTriangle size={15}/>截止目前 Gap</span><em>关注</em></div><strong>{moneyWan(gap)}</strong><small>距离H1收入预算 <i/> 已按10015单项目口径更正</small><div className="budget-kpi-rule"><b/>收入追回优先级最高</div></article>
   ...
   <article className="good"><div className="budget-kpi-label"><span><TrendingUp size={15}/>H1毛利率</span><em>有利</em></div><strong>{pct(grossMargin)}</strong><small>预算 {pct(grossMarginBudget)} <i/> 高预算 {((grossMargin-grossMarginBudget)*100).toFixed(1)}pp</small><div className="budget-kpi-rule"><b/>盈利质量高于预算线</div></article>
  </div>
  <div className="budget-body">
   ...
   <aside className="budget-focus"><div className="budget-section-head"><div><span>重点关注</span><h3>AI追回计划</h3></div><Bot size={19}/></div>{state.recoveryPlan.map(item=><article key={item.priority}><b>{String(item.priority).padStart(2,'0')}</b><div><h4>{item.title}</h4><p>{item.target}</p><small>{item.owner} · {item.rationale}</small></div></article>)}</aside>
  </div>
  ...
  {expanded&&<div className="budget-detail"><div className="budget-detail-head"><span>指标</span><span>H1预算</span><span>H1实际</span><span>完成/消耗</span><span>Gap</span><span>判断</span></div>{detailCodes.map(code=>{const metric=byCode(code),budget=valueFor(metric,'budget',state.metrics),actual=valueFor(metric,'actual',state.metrics),isPenalty=penaltyCodes.has(code),isRate=rateCodes.has(code)||['0601','0603'].includes(code);const metricGap=actual-budget;const rate=budget?actual/budget:actual>0?1:0;const favorable=isPenalty?metricGap>=0:isRate?metricGap>=0:rate>=1;return <div className="budget-detail-row" key={code}><span><b>{metric.name}</b><small>{metric.code}</small></span><span>{displayValue(metric,budget)}</span><span>{displayValue(metric,actual)}</span><span>{isPenalty?`预算消耗 ${pct(Math.abs(actual/budget))}`:pct(rate)}</span><span className={favorable?'good':'risk'}>{isRate?`${metricGap>=0?'+':''}${(metricGap*100).toFixed(1)}pp`:moneyWan(metricGap)}</span><span className={favorable?'good':'risk'}>{favorable?<><CheckCircle2 size={13}/>达标/有利</>:<><Target size={13}/>重点追回</>}</span></div>})}</div>}
  <footer><span><TrendingUp size={14}/> 毛利额实际 {moneyWan(valueFor(gross,'actual',state.metrics))} / 预算 {moneyWan(valueFor(gross,'budget',state.metrics))}</span><span>分摊后毛利率 {pct(afterMargin)} / 预算 {pct(afterMarginBudget)}</span><span>不含税收入 {moneyWan(valueFor(noTax,'actual',state.metrics))}</span><span>分摊后毛利额 {moneyWan(valueFor(afterGross,'actual',state.metrics))}</span></footer>
 </section>
}
