...
const systemRolesSeed: SystemRole[] = [
  ...
  {id:'operation-director',name:'运营总监',code:'OPERATION_DIRECTOR',level:'基地级',description:'关注基地经营、甲方动态、行业舆情、结费回款和组织效能。',memberCount:1,menus:['command','industry-news','settlement','alerts','workforce','tasks','reports','salary'],builtIn:true,status:'active'},
  ...
  {id:'team-leader',name:'客服班长',code:'TEAM_LEADER',level:'班组级',description:'负责班前会、班组看数、面谈辅导及任务闭环。',memberCount:28,menus:['command','alerts','meeting','team','workforce','tasks','reports','growth'],builtIn:true,status:'active'},
  ...
  {id:'hrbp-manager',name:'HRBP经理',code:'HRBP_MANAGER',level:'专业岗',description:'人员档案、流失预警、招聘缺口和组织效能。',memberCount:2,menus:['command','alerts','workforce','tasks','reports','salary'],builtIn:true,status:'active'},
]
...
function App(){
  ...
  const runSystemUserAction=async(id:string,action:'toggle_status'|'reset_password')=>{
   setAccessBusy(true)
   try{applyAccess(await accessApi.userAction(id,action));notify(action==='reset_password'?'密码已安全重置为000000，首次登录需改密':'账号状态已持久化更新');return true}
  ...
  const logout=async()=>{
   ...
   setAuth(null);setRole('leader');setPage('command');setRoleOpen(false);setAssistantOpen(false);setAuthError('');setAuthBusy(false)
  }
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-foot"><p>数据状态</p><div><Database size={15}/><span>模拟适配层</span><b>已连接</b></div><small>字段口径参照 ai_hack_s2<br/>最后刷新 10:18:32</small></div>
    </aside>
    ...
    <main>
      ...
      <div className="workspace" key={demoRevision}>
        {page==='command'&&(
          ...
          role==='employee'?<EmployeePage notify={notify} workflow={workflow} setWorkflow={setWorkflow} setPage={setPage}/>:
          ...
          role==='hrbp'?<HrbpPage notify={notify} cases={hrbpCases} createCase={createHrbpCase} setPage={setPage} busy={workflowBusy} workflow={workflow} run={runWorkflow}/>:
        ...
        {page==='growth'&&<GrowthPage notify={notify}/>}
    ...
    {notificationOpen&&<NotificationCenter role={role} items={roleNotifications} readIds={readNotifications} markRead={(id)=>setReadNotifications(Array.from(new Set([...readNotifications,id])))} markAll={()=>setReadNotifications(roleNotifications.map(n=>n.id))} close={()=>setNotificationOpen(false)} go={(target)=>{if(allowedMenus.includes(target)){setPage(target)};setNotificationOpen(false)}}/>}
    ...
    {assistantOpen&&<section className="assistant-panel" aria-label="AI作战助手聊天窗口">
      ...
      <div className="assistant-context"><div><Sparkles size={14}/><span>当前上下文</span><b>{currentRole.label} · {pageLabel}</b></div><button disabled={assistantBusy||assistantActionBusy} onClick={()=>{setAssistantMessages([assistantGreeting]);setAssistantDraft(null);setAssistantError('')}}>新建对话</button></div>
...
function EmployeePage({notify,workflow,setWorkflow,setPage}:{notify:(s:string)=>void;workflow:WorkflowState|null;setWorkflow:(state:WorkflowState)=>void;setPage:(page:string)=>void}){
 ...
 return <><PageHead eyebrow="个人精益作战台 · 目标牵引每一个动作" title="李倩，今天先稳产能，再守住质量" desc="用月目标拆解每日动作：产能决定贡献，质量决定贡献能否持续。" actions={activeSupport?<button className="primary" onClick={()=>setPage('tasks')}><ListChecks size={16}/>查看支持进度</button>:<button className="primary" onClick={()=>{setSupportOpen(true);setSupportSubmitted(false);setSupportRequestId('')}}><Headphones size={16}/>请求班长支持</button>}/>
  ...
  <section className="employee-focus-banner"><div className="employee-focus-title"><span>本月核心任务</span><h2>人工应答量还差 258 通，按剩余 7 个工作日每天补齐 37 通</h2><p>昨日应答 91 通、超过日目标 3 通。保持当前节奏可在月末达成，质量指标需同步守住。</p></div><div className="employee-focus-score"><div><strong>87.7%</strong><span>月度产能达成</span></div><i style={{'--employee-progress':'87.7%'} as React.CSSProperties}></i><small>当前节奏：可达成</small></div><div className="employee-principle"><Target size={22}/><div><span>目标导向法则</span><strong>先看差值 → 拆到每天 → 每小时校准</strong><p>不与别人比绝对值，只与自己的目标和昨天比进步。</p></div></div></section>
  <div className="employee-main-grid">
   <section className="panel employee-productivity"><div className="panel-head"><div><span>第一优先级 · 产能</span><h2>月度达成与昨日表现</h2></div><em>截至 7月21日</em></div><div className="employee-product-list">{productivity.map(item=><article key={item.label}><header><span>{item.label}</span><b className={item.progress>=100?'good':'risk'}>{item.progress>=100?'已达成':'追赶中'}</b></header><div className="employee-value-line"><strong>{item.value}</strong><small>/ 目标 {item.target}</small><em>{item.gap}</em></div><div className="employee-progress"><i style={{width:`${Math.min(item.progress,100)}%`}}></i></div><footer><span>昨日实际 <b>{item.yesterday}</b></span><span>昨日目标 {item.yesterdayTarget}</span><em>{item.yesterdayMet?'昨日达标':'昨日未达标'}</em></footer></article>)}</div></section>
...
function HrbpPage({notify,cases,createCase,setPage,busy,workflow,run}:{notify:(text:string)=>void;cases:HrbpCase[];createCase:(payload:Parameters<typeof workflowApi.createHrbpCase>[0])=>Promise<boolean>;setPage:(page:string)=>void;busy:boolean;workflow:WorkflowState|null;run:(action:()=>Promise<WorkflowState>,success:string)=>void}){
 ...
 const teamRisk=[{name:'普通一区·8班',rate:18.6,people:4},{name:'普通二区·3班',rate:16.2,people:3},{name:'预警专席·1班',rate:12.8,people:2},{name:'普通一区·6班',rate:9.4,people:1},{name:'高星一区·2班',rate:6.8,people:0}]
 ...
 const cycleRisk=[{cycle:'培训期',people:26,rate:15.4,target:'≤12%',risk:4,action:'强化入职预期与业务认知'},{cycle:'实操期',people:31,rate:19.3,target:'≤12%',risk:6,action:'增加带教与每日情绪反馈'},{cycle:'实习期',people:68,rate:16.2,target:'≤10%',risk:7,action:'关注绩效落差与班组融入'},{cycle:'正式期',people:361,rate:8.1,target:'≤8%',risk:5,action:'关注薪资、晋升与工作负荷'}]
 const riskEmployees=[
 ...
 const createCommunicationTask=async()=>{
  ...
  const created=await createCase({role:'hrbp',actor:'HRBP经理 · 陈静',plan:communicationPlan,due:'今日 16:00',employee:{id:selectedEmployee.id,name:selectedEmployee.name,team:selectedEmployee.team,batch:selectedEmployee.batch,cycle:selectedEmployee.cycle,riskScore:selectedEmployee.score,reasons:[...selectedEmployee.signals]}})
 ...
 return <><PageHead eyebrow="呼叫中心 HRBP 作战台 · 稳定组织、提前干预" title="先看哪里在流失，再看谁可能离开，最后把干预做成闭环" desc="从班组、入职批次和员工周期识别风险；重点员工进入沟通任务，必要时升级经理PDCA。" actions={<><button className="secondary" onClick={()=>setPage('tasks')}><ListChecks size={16}/>沟通任务 {cases.filter(item=>item.status!=='closed').length}</button><button className="primary" onClick={()=>openCommunication(riskEmployees[0])}><UserRoundSearch size={16}/>新建重点沟通</button></>}/>
...
function MeetingPage({notify,workflow}:{notify:(s:string)=>void;workflow:WorkflowState|null}){
 ...
 const todayTargets=['人工应答量≥1,360','满意率≥97.2%','一次解决率≥90.0%','重复来电率≤4.0%','置忙小休≤12.0%','签入率≥96.0%']
 const agenda=[
...
function TeamPage({notify}:{notify:(s:string)=>void}){
...
function WorkflowTasksPage({role,state,error,busy,run}:{role:Role;state:WorkflowState|null;error:string;busy:boolean;run:(a:()=>Promise<WorkflowState>,s:string)=>void}){
 ...
 return <>
  <PageHead eyebrow="真实状态机 · 刷新不丢失" title={role==='quality'?'质检协同复检中心':role==='employee'?'我的支持请求':'PDCA动态任务中心'} desc={role==='quality'?'接收班长辅导证据，复检通过后闭环；未通过则退回班长继续整改。':role==='employee'?'查看班长处理结果，确认问题是否真正解决；未解决可带着补充说明退回。':'员工、质检或主管发起任务，班长执行并提交证据，责任岗位验证，所有动作全过程留痕。'} actions={<button className="secondary" disabled={!canEscalate||busy} title={!canEscalate?'协同任务不走普通逾期升级链路':`将升级至${nextRole}`} onClick={()=>run(()=>workflowApi.simulateTimeout(role),`已模拟SLA逾期，任务升级至${nextRole}`)}><Clock3 size={15}/>{canEscalate?`模拟逾期→${nextRole}`:'当前不可升级'}</button>}/>
  <div className="workflow-stagebar">{[['P',isAiTask?'AI建议确认':isMeetingTask?'会议下发':'问题触发'],['D',isAiTask?`${roleLabels[task?.originRole||role]}执行`:isMeetingTask?'责任岗位执行':'班长响应'],['C',isMeetingTask?'总监验收':isAiTask?`${verificationLabel}验收`:isQualityTask?'质检复检':isEmployeeSupport?'员工确认':'主管验证'],['A','关闭/升级']].map(step=><div className={task&&(task.phase===step[0]||step[0]==='P')?'active':''} key={step[0]}><b>{step[0]}</b><span>{step[1]}</span></div>)}</div>
...
function UserManagementPage({users,saveUser,userAction,roles,notify,busy,error}:{users:SystemUser[];saveUser:(user:SystemUser)=>Promise<boolean>;userAction:(id:string,action:'toggle_status'|'reset_password')=>Promise<boolean>;roles:SystemRole[];notify:(s:string)=>void;busy:boolean;error:string}){
 ...
 return <><PageHead eyebrow="系统管理 · 账号与权限" title="用户管理" desc="维护用户账号、岗位、角色、联系方式与角色外模块配置，确保人员信息完整、权限可追溯。" actions={<button className="primary" onClick={()=>setEditing({id:nextUserId(),name:'',jobNo:'',roleId:'customer-agent',jobTitle:'客服专员',department:'河北基地',phone:'',email:'',status:'active',password:'000000',forceChangePassword:true,moduleOverrides:[],createdAt:'2026-07-22'})}><UserPlus size={16}/>新增用户</button>}/>
...
function SalaryPage(){return <><PageHead eyebrow="员工看得懂 · 班长算得清" title="绩效变化，实时映射到薪资" desc="展示每一项工资依据，但严格遵循字段级权限：员工只看本人，班长看绩效不看实发工资。"/><div className="salary-layout"><section className="salary-hero"><span>本月预估实发</span><strong>¥ 6,842</strong><p>按当前绩效与出勤进度预估 · 置信区间 ¥6,680—¥7,020</p><div><span><b>¥3,800</b>基本工资</span><span><b>¥1,742</b>绩效工资</span><span><b>¥860</b>业务激励</span><span><b>¥440</b>补贴奖惩</span></div></section><section className="panel salary-detail"><div className="panel-head"><div><span>字段级可追溯</span><h2>薪资构成依据</h2></div><ShieldCheck size={20}/></div>{[['基本工资','¥3,800','人工成本表 · JBGZ_*'],['绩效工资','¥1,742','人工成本表 · JXGZ / JXXS / SFJX'],['业务激励','¥860','特殊奖励 · JLJE + 营销佣金'],['夜班与培训补贴','¥380','YBBZ + PXBT'],['考勤与奖惩','¥60','员工月考勤 + JCXJ']].map(x=><div className="salary-row" key={x[0]}><div><strong>{x[0]}</strong><small>{x[2]}</small></div><b>{x[1]}</b></div>)}</section></div></>}
